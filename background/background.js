...
const agentState = {
  ...
  history: [],
  lastThinking: "",
  // Task plan: short high-level decomposition of the goal generated by a one-time
  // LLM call at task start. Acts as a persistent context anchor — injected into
  // every subsequent agent prompt so the LLM doesn't lose sight of the bigger
  // picture as history grows. Empty array = planning failed or skipped.
  plan: [],
  logs: [],
  interStepDelay: 2000,
  llmTimeout: 100000,
...
- Only use "plan" when you are highly confident about the sequence. For uncertain situations, return a single action.`;
  ...
  // Render the persistent task plan (if any) — anchored above the page state
  // so it's the first thing the LLM sees after the goal. Doesn't decay with
  // history; same on every step of this task.
  const planBlock = (agentState.plan && agentState.plan.length > 0)
    ? "\n\nYOUR PLAN (created at task start — your north star; deviate only if reality clearly differs):\n" +
      agentState.plan.map((s, i) => `  ${i + 1}. ${s}`).join("\n")
    : "";

  let user = `Goal: "${goal}"${planBlock}
...
{ "action": "error", "reason": "<why you cannot proceed>", "completed": "<what you did accomplish before getting stuck>", "manual_steps": "<what the user should do to finish the task>" }`;
  ...
  // Render the persistent task plan (same anchor used in the DOM prompt)
  const planBlock = (agentState.plan && agentState.plan.length > 0)
    ? "\n\nYOUR PLAN (created at task start — your north star; deviate only if reality clearly differs):\n" +
      agentState.plan.map((s, i) => `  ${i + 1}. ${s}`).join("\n")
    : "";
...
async function _attemptVisionOnce(tabId, goal, reason, vm, somCap = 80) {
  ...
  let action;
  try {
    action = await callLLM(msgs, vm, "Vision", visionScreenshot, { timeout: 125000 });
  } catch (err) {
...
/* ═══════════════════════════════════════════
 * TASK PLANNING — one-time LLM call at task start
 *
 * Generates a short high-level plan (3-7 steps) from the user's goal. The plan
 * is stored in agentState.plan and injected into every subsequent agent prompt
 * as a persistent context anchor — so the LLM doesn't lose sight of the bigger
 * picture as the action history accumulates over many steps.
 *
 * Failures are non-fatal: if the planning call errors out, agentState.plan
...
- No code fences. No commentary. JSON only.`;
  ...
  try {
    const result = await callLLM(
      ...
      "TaskPlan",
      null,
      { forceJson: true, timeout: 20000 }
    );

    // callLLM returns parsed JSON when forceJson is true, otherwise raw content
    let parsed = result;
    if (typeof result === "object" && result._rawContent && !result.plan) {
      // Some providers return the JSON nested under _rawContent — try to parse
      try { parsed = JSON.parse(result._rawContent); } catch {}
    }

    const plan = Array.isArray(parsed && parsed.plan) ? parsed.plan : [];
    // Sanitize: keep only string entries, trim, drop empties, cap to 7
    const clean = plan
    ...
    broadcastLog({
      ...
      data: { steps: clean.length, plan: clean }
    });
    ...
    broadcastLog({
      kind: "system",
      label: "Plan Generation Failed (continuing without plan)",
...
async function runAgentLoop(goal, tabId) {
  ...
  agentState.history = [];
  agentState.lastThinking = "";
  agentState.plan = []; // Cleared every new task; populated by generateTaskPlan below
  agentState.logs = [];
